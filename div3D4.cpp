...
ll getVal(int n, ll x, ll y) {
    ...
    return quadrant * block + getVal(n - 1, newx, newy);
}

pll getCoord(int n, ll d) {
    if(n == 1) {
        if(d == 1) return {1, 1};
        if(d == 2) return {2, 2};
        if(d == 3) return {2, 1};
    ...
    ll half = 1LL << (n - 1);
    ll block = 1LL << (2 * (n - 1));
    int quadrant = (d - 1) / block;
    ll rem = d - quadrant * block;
    pll p = getCoord(n - 1, rem);
    ll x = p.ff, y = p.ss;
    if(quadrant == 0) return {x, y};   
    if(quadrant == 1) return {x + half, y + half};  
    if(quadrant == 2) return {x + half, y};        
    return {x, y + half};
}
