...
void solve() {
    int l, r, i, k;
        ...
        int64_t highRangeStart = (l - k + (1ULL << i) - 1) >> i;
        int64_t highRangeEnd = (r - k) >> i;
        int64_t highBits = xor_range(highRangeStart, highRangeEnd) << i;
div
        int64_t count = (r - k) / (1ULL << i) - (l - k - 1) / (1ULL << i);
        int64_t lowBits = (count & 1) ? k : 0;
