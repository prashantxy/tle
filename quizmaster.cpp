...
vector<int> getDivisors(int x, int m) {
    vector<int> divs;
    for (int d = 1; d * d <= x; d++) {
        if (x % d == 0) {
            if(d <= m)
...
void solve() {
    int n, m;
    cin >> n >> m;
    vector<int> smartness(n);
    for (int i = 0; i < n; i++){
        cin >> smartness[i];
    }
    vector<bool> globalCovered(m+1, false);
    vector<vector<int>> divisors(n);
    for (int i = 0; i < n; i++){
        divisors[i] = getDivisors(smartness[i], m);
        for (int d : divisors[i]){
            globalCovered[d] = true;
        }
    }
    bool possible = true;
    for (int T = 1; T <= m; T++){
    ...
    for (int i = 0; i < n; i++){
        students.push_back({smartness[i], i});
    }
    ...
    int l = 0;
    for (int r = 0; r < n; r++){
        int curSmart = students[r].first;
        int idx = students[r].second;
        ...
        while(coveredCount == m && l <= r){
            ans = min(ans, students[r].first - students[l].first);
            int leftIdx = students[l].second;
            for (int d : divisors[leftIdx]){
                freq[d]--;
                if (freq[d] == 0) {
                    coveredCount--;
                }
